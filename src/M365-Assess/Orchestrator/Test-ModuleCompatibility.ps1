function Test-ModuleCompatibility {
    ...
    # msalruntime.dll ΓÇö Windows only, EXO 3.8.0+ may ship the DLL in a nested path
    ...
    if ($needsExo -and -not $exoModule) {
        ...
        $repairActions.Add([PSCustomObject]@{
            ...
            InstallCmd      = 'Install-Module -Name ExchangeOnlineManagement -Scope CurrentUser -Force'
    ...
    if ($repairActions.Count -eq 0) {
        ...
        if ($NonInteractive -or -not [Environment]::UserInteractive) {
            ...
            # --- Headless: auto-fix FileCopy issues first ---
            ...
            foreach ($action in $fileCopyActions) {
                try {
                    ...
                    Write-AssessmentLog -Level INFO -Message "Copied msalruntime.dll to EXO module load path"
            ...
            if ($requiredIssues.Count -gt 0) {
                ...
                Write-Host '  Minimum required: Graph SDK 2.36.0+' -ForegroundColor DarkGray
            ...
            if ($needsExo -and -not $exoModule) {
                ...
                $stillBroken += 'Install-Module -Name ExchangeOnlineManagement -Scope CurrentUser -Force'

            }



            # Re-check msalruntime.dll after any EXO install
